...
Cele szczegółowe:
\begin{itemize}
        \item Szesnasto-kanałowy odczyt i zliczenie impulsów o częstotliwości osiągającej do 2.5$\frac{MHz}{kanał}$ z wykorzystaniem mikrokontrolera \textit{Arduiono Due}.
...
\subsection{Czas egzekucji programu}
...
W celu obliczenia ilości cykli które procesor wykonuje w przeciągu sekundy ($\frac{Czas}{Cykl}$) można skorzystać z równości:
\begin{equation}
        \label{Cykli w sec}
        N [1]= \frac{1}{T [Hz]}
\end{equation} 
...
%piping i niskie działanie procesora
%iron law
%mulithreading programing deadlock's
%typy plików z danymi
%pipelining
%microarchitektura i ISA
\section{Projekt}

...
Układ elektroniki wraz z detektorem służący do recepcji sygnału promieniowania jonizującego nazywany jest elektroniką front-end. Przykładem konstrukcji tego typu jest projekt RXHDR\_V1. Jest to układ scalony dzielący się na 2 części: krzemowy detektor paskowy (detektor którego przestrzenią aktywną są podłużne spolaryzowane krzemowe diody p-n) \cite{master} \cite{front-end} oraz układ elektroniki kształtującej. Konstrukcja projektu pozwala na odczyt informacji deponowanej w detektorze w dwóch formach poprzez implementację podwójnego systemu kształtowania  impulsu:
\begin{itemize}
        \item Analogowy system integracji który sumuję energię deponowaną w detektorze i przekazuje tą informację w postaci napięcia na wyjście układu.
        \item System cyfrowy który po ukształtowaniu impulsu i dyskryminacji przekazuje informacji o wystąpieniu sygnału za pomocą impulsu prostokątnego.    
\end{itemize} 

Diagram blokowy układu kształtowania impulsu znajduje się na rysunku \ref{RXHDR schema}. Układ cyfrowy przed wytworzeniem impulsów o szerokości 20-200ms i wysokości 1.8 V przechodzi, przez następujące etapy kształtowania:
\begin{itemize}
        \item Przedwzmacniacz ładunkowy, pierwszy krok w procesie odbioru sygnału mający na celu wzmocnienie sygnału przed dalszym kształtowaniem,
        ...
        \item Dwuczłonowy układ kształtowania typu T, etap kształtowania impulsu Gauso-podobnego oraz miejsce dodatkowego wzmocnienia. 
        \item Dyskryminator progowy, w tym miejscu sygnał jest transformowany z analogowego napięcia na binarny impuls 0V, 1.8V. Szerokość impulsu mówi o energii impulsu.
\end{itemize}
...
Każdy z 32 padów odczytowych, poza bezpośrednim połączeniem z paskiem detektora, jest podłączony do pojemności testowej pozwalającej na symulację zdarzeń deponowania fotonu za pomocą generowanego sygnału prostokątnego. Architektura poza dostarczeniem wymaganych prądów polaryzacyjnych pozwala na zewnętrzne ustalenie progu dyskryminacji.

\begin{figure}
        \includegraphics[width=\textwidth]{RXHDR_shaper.png}
        \caption{Schemat blokowy układu kształtującego projektu RXHDR\_V1}
        \label{RXHDR schema}
\end{figure}
